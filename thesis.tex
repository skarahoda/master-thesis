...
quality of the synchronizing words generated by the heuristic, we propose two main techniques that speedup \greedyAlgo . First, we focused on parallelization of \textsc{Greedy}. Second, we propose a lazy execution of the preprocessing, by postponing the preparation of the required information until it is to be used in the reset word generation phase. We suggest other algorithmic enhancements as well for the implementation of these heuristics. 

To the best of our knowledge, this is the first work towards parallelization of  synchronizing heuristics. Although, a parallel approach for constructing a synchronizing sequence for automata with partial transition function \kkcomm{define partial} \comment{sertac}{introda automatanin tanimini dahi vermedigimiz icin partial machinesin defininitionini nasil veririm tam emin olamadim ondan partial transition function diye daha kolay anlasilir yapmaya calistim} has been proposed in \cite{Uraz}, it is not exact (in the sense that it may fail to find a synchronizing sequence even if at least one exists). Furthermore, it is not a polynomial time algorithm.
...
\[
  \delta_c(s_i, x) = \left.
  \begin{cases}
    s_{i+i\ (\mathrm{mod}\ n)}, & x = b \text{ or } s_i = s_0\\
    s_i, & \text{otherwise}
  \end{cases} \right.
\]
then the length of the shortest reset word of ${\cal A}_c$  is $(n-1)^2$. Since the \v{C}ern\'y conjecture is not disproven yet the shortest reset word of ${\cal A}_c$ is known as the longest among the shortest reset words ofslow automata with the same number of states. In this thesis, ${\cal A}_c$ is called as the \v{C}ern\'y automaton. 
