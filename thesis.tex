...
\begin{algorithm}[ht]
	...
	\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
	\Input{An automaton $A=(S,\Sigma,\delta)$, the frontier level $f$, $\tau$}
	\Output{updated function $\tau$}
	
	\ForEachP{$\langle s_i,s_j\rangle \in S^2$}
	{
		\If{$|\tau(\langle s_i, s_j\rangle)| = f$}{
		\ForEach{$x \in \Sigma$}
		{
			\ForEach{$\langle s'_i,s'_j\rangle$ where $s'_i \in \delta^{-1}(s_i,x)$ and $s'_j \in \delta^{-1}(s_j,x)$}
			{
...
\begin{algorithm}[ht]
	...
	\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
	\Input{An automaton $A=(S,\Sigma,\delta)$, $\tau$}
	\Output{updated function $\tau$}
	
		\ForEachP{$\langle s_i,s_j\rangle \in S^2$}
		{
			\If{$\tau(\langle s_i, s_j\rangle)$ is undefined}{
			\ForEach{$x \in \Sigma$}
			{
				$\langle s'_i, s'_j \rangle \longleftarrow \langle \delta(s_i,x),\delta(s_j,x) \rangle$; \\ 
...
\begin{algorithm}[ht]	
	...
	\lForEach{singleton $\langle s,s \rangle \in S^{\langle 2 \rangle}$}{$\tau(\langle s,s \rangle) = \varepsilon$}
	\lForEach{pair $\langle s_i,s_j \rangle \in S^{\langle 2 \rangle}$}{$\tau(\langle s_i,s_j \rangle) = \mbox{\em undefined}$}
	$Q \longleftarrow \{ \langle s,s\rangle | s \in S \}$; \tcp{Q is a queue which will store unprocessed pair from frontier set and found pair from next frontier set.}
	$C = S$; \tcp{$C$ will keep track of the current set of states}
	$\Gamma = \varepsilon$; \tcp{$\Gamma$ is the synchronizing sequence to be constructed}
	\While{$|C| > 1$}	
	{
		\While{$\forall \langle s_i,s_j \rangle \in C^{\langle 2 \rangle}: \tau(\langle s_i,s_j \rangle)$ is undefined}
		{
			$\langle s_i, s_j \rangle = $ dequeue the next item from $Q$;\\
			\ForEach{$x \in \Sigma$}
			{
				\ForEach{$\langle s_k, s_\ell \rangle \in \delta^{-1}(\langle i,j \rangle,x)$}
				{
					\If{$\tau(\langle s_k, s_\ell \rangle)$ is undefined}
					{
