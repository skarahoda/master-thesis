...
Bilgisayar Bilimi ve M\"{u}hendisli\u{g}i, Y\"{u}ksek Lisans Tezi, 2018
...
\begin{center}
Anahtar Kelimeler: Sonlu durum otomatlar{\i}, S{\i}f{\i}rlama kelimeleri, S{\i}f{\i}rlama Sezgiselleri, A\.{I}\"{U} paralelle\c{s}tirilmesi, G\.{I}\"{U} paralelle\c{s}tirilmesi
\end{center}
...
{\bf \"{O}zet}
\end{center}
Karma\c{s}{\i}k sistemlerin geli\c{s}tirilmesinde, test etme en pahal{\i}  ve en \c{c}ok zaman alan evredir. Model tabanl{\i} testler y\"{u}ksek kaliteli deney kurgusunu otomatik \"{u}retmede kullan{\i}lan yakla\c{s}{\i}mlardan birisidir. Deney kurgusunu otomatik \"{u}retme test etmenin en zorlu par\c{c}alar{\i}ndan biridir. Sonlu durum makineleri ya da \"{o}zdevinimler gibi bi\c{c}imsel modeller, otomatik deney grubunu \"{u}retmek i\c{c}in kullan{\i}lmaktad{\i}r. Sistem  belirli bir duruma senkronize edildikten sonra testler uygulan{\i}r ve bu belirli duruma gelebilmek i\c{c}in s{\i}f{\i}rlama kelimeleri kullan{\i}lmaktad{\i}r. Daha k{\i}sa deney s\"{u}releri i\c{c}in en k{\i}sa s{\i}f{\i}rlama kelimesini hesaplamak \"{o}nemlidir, ancak en k{\i}sa s{\i}f{\i}rlama kelimesini hesaplamak NP--hard bir problemdir. Bu nedenle k{\i}sa s{\i}f{\i}rlama kelimelerini hesaplamak i\c{c}in sezgisel y\"{o}ntemler kullan{\i}lmaktad{\i}r. \textsc{Greedy} algoritmas{\i} bu alanda bilinen en h{\i}zl{\i} sezgisel algoritmad{\i}r. Bu tezde, \textsc{Greedy} algoritmas{\i}n{\i} h{\i}zland{\i}ran yakla\c{s}{\i}mlar sunulmaktad{\i}r. \.{I}lk olarak \textsc{Greedy} algoritmas{\i}n{\i}n paralelle\c{s}tirilmesine odaklan{\i}lmaktad{\i}r. \.{I}kinci olarak ise tembel bir yakla\c{s}{\i}m \"{o}nererek s{\i}f{\i}rlama kelimesinin \"{u}retilmesi i\c{c}in gerekli bilgilerin haz{\i}rlanma s\"{u}reci ertelenmektedir. Ayn{\i} zamanda, \textsc{Greedy} algoritmas{\i} i\c{c}in benzer algoritmik iyile\c{s}tirilmeler \"{o}nerilmektedir. Deney sonu\c{c}lar{\i}m{\i}z \"{o}zdevinim b\"{u}y\"{u}kl\"{u}\u{g}\"{u}ne ba\u{g}l{\i} olarak \textsc{Greedy} algoritmas{\i}n{\i}n 500 kat daha h{\i}zl{\i} hale getirilebilece\u{g}ini g\"{o}stermektedir. \"{O}nerilen geli\c{s}tirmeler \"{o}zdevinim b\"{u}y\"{u}kl\"{u}\u{g}\"{u} artt{\i}k\c{c}a daha etkili hale gelmektedir.
...
I would like to state my gratitude to my supervisors, H\"{u}sn\"{u} Yenig\"{u}n and Kamer Kaya for everything they have done for me, especially for their invaluable guidance, limitless support and understanding.
...
have bundled into several warps.  Each thread executes a
...
%		\v{C}ern\'y & 1999000.0 & 1952000.0 & 8884.8 & 7998000.0 & 7808000.0 & 19750.8 & 31996000.0 & 31232000.0 & 43480.8
	\end{tabular}
	\caption{The length of longest merging sequence in PMF($h_{PMF}$) constructed in the first phase for random automata; maximum ($h_{max}$), and average ($h_{mean}$) lengths for merging sequences, used in the second phase of \greedyAlgo.}
	\label{table:levels}
\end{table}
...
\begin{table}[ht]
	...
	\end{tabular}
	\caption{The length of longest merging sequence in PMF($h_{PMF}$) constructed in the first phase for the \v{C}ern\'y automata; maximum ($h_{max}$), and average ($h_{mean}$) lengths for merging sequences, used in the second phase of \greedyAlgo .}
...
To store and utilize the  $\delta^{-1}(s,x)$ for all $x \in \Sigma$ and $s \in S$, we employ the data structures in Fig.~\ref{fig:inv}~(right). For each symbol $x \in \Sigma$, we used two arrays ${\tt ptrs}$ and ${\tt ids}$ where the former is of size $n + 1$ and the latter is of size $n$. For each state $s \in S$, ${\tt ptrs}[s]$ and ${\tt ptrs}[s+1]$ are the start~(inclusive) and end~(exclusive) pointers to two ${\tt ids}$ entries.  The array ${\tt ids}$ stores the ids of the states $\delta^{-1}(s,x)$ in between ${\tt ids}$[{$\tt ptrs$}[$s$]]  and ${\tt ids}$[{$\tt ids$}[$s+1$] - 1]. This representation has a low memory footprint. Furthermore, we access the entries in the order of their array placement  in our implementation hence, it is also good for spatial locality. We also sorted the set of current pairs by their indexes before line 2 of Algorithm \ref{algo:find-min-parallel} since it improves spatial locality further.
