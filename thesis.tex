...
In this work we investigate the use of modern multicore CPU and GPUs to scale the performance of synchronizing heuristics. We consider the \textsc{Greedy} algorithm to start with, as it is one of the two cheapest synchronizing heuristics (in practice as well \cite{RS15}), known to produce shorter sequences than \textsc{Cycle} \cite{RS15}, and has been widely used as a baseline to evaluate the quality and speed of more advanced heuristics.  To the best of our knowledge, this is the first work towards parallelization of  synchronizing heuristics. Although, a parallel approach for constructing a synchronizing sequence for a {\em partial}  machines is proposed in \cite{Uraz}, the method proposed in \cite{Uraz} is not exact (in the sense that it may fail to find a synchronizing sequence even if one exists) and also it is not a polynomial
time algorithm.
...
The rest of the thesis is organized as follows: In Section~\ref{sec:Preliminaries}, the notation is given, and synchronizing sequences are formally defined. We give the details of Eppstein's {\sc Greedy} construction algorithm in Section \ref{sec:greedy}. The parallelization approach together with implementation details are described in Section \ref{sec:parallel}.
In Section \ref{sec:speedup}, algorithmic speedups that avoid redundant computations are introduced. Section \ref{sec:results} presents the experimental results and Section~\ref{sec:conclusion} concludes the paper.
...
An automaton which produced from set of pairs; ${\cal A}^{\langle 2 \rangle}=(S^{\langle 2 \rangle},\Sigma,\delta^{\langle 2 \rangle})$ is called the \textit{pair automaton}. For a pair automaton, the set of inputs are the same and the transition function is $\delta^{\langle 2 \rangle}(\{ s_i,s_j \},x) = \{ \delta(s_i,x), \delta(s_j,x) \}$. 
...
\textsc{Greedy} is widely used as a performance baseline for the new heuristics in the literature. None of the newer heuristics could actually match the time performance of \textsc{Greedy} and \textsc{Cycle} so far. With the new improved approaches suggested in this paper, \textsc{Greedy} and \textsc{Cycle} have now become more competitive than they already are.
